...
def killSession(id, hard=False):
    ...
    if hard or todelete["lastaccessed"] < now - (inactive_timeout * 60):
        ...
        killtime = datetime.datetime.now() + datetime.timedelta(
            minutes=inactive_timeout
        )
...
def vote(publicID):
    ...
    session_db.update_one(
        {"publicID": publicID}, {"$set": {"lastaccessed": now, "lastread": now}}
    )
    current_playlist = list(
        setlist_db[publicID].find({"played": 0}, {"_id": 0}).sort("upvotes", -1)
    )
    ...
    if current_playlist != []:
        ...
        while len(tmp_playlist) >= 49:
            tracks += sp.tracks([x["uri"] for x in tmp_playlist[:45]])["tracks"]
        ...
        for t in range(len(tracks)):
            ...
            if publicID + "-guest" in request.cookies:
                if (
                    "upvoters" in curr_track.keys()
                    and request.cookies[publicID + "-guest"] in curr_track["upvoters"]
                ):
                    curr_track["vote"] = "up"
                    # curr_track["upvotes"] -= 1
                elif (
                    "downvoters" in curr_track.keys()
                    and request.cookies[publicID + "-guest"] in curr_track["downvoters"]
                ):
    ...
    resp = make_response(
        render_template("vote.html", publicID=publicID, tracks=current_playlist)
    )
    ...
    if (publicID + "-guest") not in request.cookies:
        ...
        if session_info["settings"]["captcha"]:
            return redirect(
                "https://www.crowdsourcejukebox.com/captcha/" + publicID + "/"
            )
        else:
...
def search(publicID):
    ...
    if (publicID + "-guest") not in request.cookies:
        ...
        if not hancock.validate(request.cookies.get(publicID + "-guest")):
            abort(401)
    if (
        session_info["settings"]["songlimit"]
        and setlist_db[publicID].find_one(
            {"played": 0, "submitted_by": request.cookies.get(publicID + "-guest")}
        )
        is not None
    ):
    ...
    if query != "" and "query=" in query:
        ...
        return render_template(
            "results.html", tracks=tracks, publicID=publicID, prev_query=query
        )
    else:
        return render_template("search.html", publicID=publicID, wait=False)
...
def captcha(publicID):
    ...
    if request.method == "POST":
        ...
        if is_human(captcha_response):
            resp = make_response(
                redirect("https://www.crowdsourcejukebox.com/vote/" + publicID + "/")
            )
            guestID = bytes(str(uuid.uuid4()), "utf-8")
            session_db.update_one({"publicID": publicID}, {"$inc": {"guests": 1}})
            resp.set_cookie(
...
def api():
    ...
    if form["req"] == "playnext":
        ...
        return_obj = list(
            setlist_db[publicID].find({"played": 0, "upvotes": {"$gte": 0}})
        )
        if return_obj == []:
            past_played = [
            ...
            for i in range(4):
                seeds.append(
                    top_played.pop(random.randint(0, len(top_played) - 1))["uri"]
                )
            ...
            setlist_db[publicID].update_one(
                {"_id": return_obj.pop("_id")}, {"$set": {"played": 1}}
            )
            session_db.update_one(
                {"privateID": form["privateID"]},
                {"$set": {"lastaccessed": now, "lastmodified": now}},
            )
    ...
    if form["req"] == "setlist":
        ...
        return_obj = (
            return_obj
            if form["number"] > len(return_obj)
            else return_obj[: form["number"]]
        )
        session_db.update_one(
            {"publicID": form["publicID"]},
            {"$set": {"lastaccessed": now, "lastread": now}},
        )
    if form["req"] == "updates":
        now = int(time.time())
    ...
    if form["req"] == "submit":
        ...
        if (
            session_info["settings"]["songlimit"]
            and setlist_db[publicID].find_one({"played": 0, "submitted_by": guestID})
            is not None
        ):
            abort(403)
        entry = setlist_db[publicID].find_one({"played": 0, "uri": uri})
    ...
    if form["req"] == "settings":
        ...
        session_db.update_one(
            {"privateID": form["privateID"]},
            {"$set": {"lastaccessed": now, "lastmodified": now, "settings": settings}},
        )
        return_obj = session_db.find_one({"privateID": form["privateID"]}, {"_id": 0})
        if settings["noexplicit"]:
            tmp_playlist = list(
                setlist_db[session_info["publicID"]]
                .find({"played": 0}, {"_id": 0})
                .sort("upvotes", -1)
            )
            tracks = []
